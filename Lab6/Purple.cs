...
namespace Lab6
{
    public class Purple
    {
        ...
        public void Task2(ref int[,] A, int[,] B)
        {
            ...
            if (A.GetLength(1) == B.GetLength(0))
            {
                ...
                if (k > 0)
                {
                    ...
                    {
                        k = CountPositiveElementsInRow(A, i);
                        if (k > MaxR)
                        {
                            MaxR = k;
                            maxRow = i;
        ...
        public void Task4(int[,] A, int[,] B)
        {
            ...
            int posA = FindMaxIndex(arrayA);
            int posB = FindMaxIndex(arrayB);
            if (posA != -1 && posB != -1 && A.GetLength(1) == B.GetLength(1))
            {
                for (int i = 0; i < A.GetLength(1); i++)
                {
                    (A[posA, i], B[posB, i]) = (B[posB, i], A[posA, i]);
                }
            }
            // end
